...
describes what users get. Pick a format with `-o`, or let datacite choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
datacite <command> -o table   # aligned columns for reading
datacite <command> -o jsonl   # one JSON object per line, for piping
datacite <command> -o json    # a single JSON array
datacite <command> -o csv     # spreadsheet friendly
datacite <command> -o tsv     # tab-separated
datacite <command> -o url     # just the URL column
datacite <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
datacite <command> --fields id,title,url
```
...
```bash
datacite <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
datacite <command>            # a table, because this is a terminal
datacite <command> | wc -l    # JSONL, because this is a pipe
```
